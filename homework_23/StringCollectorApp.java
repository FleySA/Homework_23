...
public class StringCollectorApp {
    ...
    public static void main(String[] args) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        List<String> strings = Arrays.asList("com.proftelran.org.homework_23.StudentBE",
                "com.proftelran.org.homework_23.StudentFE",
                "com.proftelran.org.homework_23.StudentQA");
...
// Обходим каждого студента и собираем информацию о его полях
        for (AbstractStudent student : studentList) {
            Class<?> studentClass = student.getClass();
            if (studentClass.isAnnotationPresent(StudentPrefix.class)) {
                StudentPrefix studentPrefix = studentClass.getAnnotation(StudentPrefix.class);
                String classPrefix = studentPrefix.value();
                Field[] fields = studentClass.getDeclaredFields();
                for (Field field : fields) {
                    if (field.isAnnotationPresent(FieldPrefix.class)) {
                        FieldPrefix fieldPrefix = field.getAnnotation(FieldPrefix.class);
                        field.setAccessible(true);
                        String fieldName = field.getName();
                        String fieldValue = (String) field.get(student);
                        resultBuilder.append(classPrefix).append(": ").append(fieldPrefix.value()).append(": ").append(fieldValue).append("\n");
                    }
                }
            }
        }
        ...
        // Создаем объект Result и вызываем метод printStudentsInfo с результирующей строкой
        Result resultObj = new Result();
        Method printMethod = Result.class.getDeclaredMethod("printStudentsInfo", String.class);
        printMethod.setAccessible(true);
        printMethod.invoke(resultObj, resultBuilder.toString());

        System.out.println("Результат сбора информации о студентах:");
